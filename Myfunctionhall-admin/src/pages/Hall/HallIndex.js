...
function Sliderindex() {
  ...
  function fetchData() {
    axiosInstance.get('http://127.0.0.1:8000/api/Hall')
      .then((res) => {
        const data = res.data;
        console.log(data);
        setShowdata(data);
  ...
  function deletedata(id) {
    axiosInstance.delete('http://127.0.0.1:8000/api/Hall/' + id)
      .then((res) => {
        const data = res.data;
        console.log(data);
        if (data) {
  ...
  {Showdata.map((row) => {
    return (
      <tr key={row.id}>
        ...
        <td>{row.address}</td>
        <td>{row.intakecap}</td>
        <td>{row.parkingcap}</td>
        <td>{row.kitchen}</td>
        <td>{row.kitchen_material}</td>
        <td>{row.timings}</td>
        <td>{row.images}</td>
        <td>{row.video}</td>
        <td>{row.contact}</td>
        ...
        <td>
          <div class="dropdown">
            ...
            <div class="dropdown-menu">
              <a class="dropdown-item" href={'/editHall/' + row.id}>
