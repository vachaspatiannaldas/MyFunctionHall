...
function ProfileIndex() {
  ...
  function deletedata(id) {
    axiosInstance.delete('http://127.0.0.1:8000/api/Profile/' + id)
      .then((res) => {
        const data = res.data;
        console.log(data);
        if (data) {
                          ...
                          {Showdata.map((row) => {
                            return (
                              <tr key={row.id}>
                                ...
                                <td>{row.category}</td>
                                <td>
                                <img src={`http://localhost:8000/profile_upload/${row.image}`} 
                                alt="#" style={{width:"380px",height:"290px",marginTop:"40px",marginLeft:"40px"}}/>
                                </td>
                                <td>
                                <img src={`http://localhost:8000/profile_upload/${row.banner}`} 
                                alt="#" style={{width:"380px",height:"290px",marginTop:"40px",marginLeft:"40px"}}/>
                                </td>
                                <td>{row.address}</td>
                                <td>{row.city}</td>
                                ...
                                <td>
                                  <div class="dropdown">
                                    ...
                                    <div class="dropdown-menu">
                                      ...
                                      <a  className='btn btn-primary' href={'/profileedit/'+row.id}>Edit</a>
